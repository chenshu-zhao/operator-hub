...
ALTER_TABLE_DICT = [
    ...
    ["dip_data_operator_hub", "t_mcp_server_config",       "INDEX",   "ADD",     "idx_update_time",   "f_update_time",         "更新时间索引"],
    ["dip_data_operator_hub", "t_mcp_server_config",       "INDEX",   "ADD",     "idx_status",   "f_status",         "状态索引"],
    ["dip_data_operator_hub", "t_mcp_server_release",      "INDEX",   "ADD",     "idx_status_update_time",   "f_status, f_update_time",         "状态更新时间索引"],
    ["dip_data_operator_hub", "t_operator_release",       "INDEX",   "ADD",     "idx_status_update_time",   "f_status, f_update_time",         "状态更新时间索引"]
]
# ！！！以下注释不可删除
# === TEMPLATE START ===
import os
...
def add_index(conn_cursor, db_name, table_name, index_name, obj_props, index_type):
    conn_cursor.execute(f'CREATE {index_type} IF NOT EXISTS idx_{table_name}_{index_name} ON `{db_name}`.`{table_name}`({obj_props});')


def drop_index(conn_cursor, db_name, table_name, index_name):
    conn_cursor.execute(f'DROP INDEX IF EXISTS `{db_name}`.idx_{table_name}_{index_name} CASCADE;')
