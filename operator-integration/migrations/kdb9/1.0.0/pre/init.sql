...
CREATE TABLE IF NOT EXISTS `t_metadata_api` (
    ...
    `f_summary` VARCHAR(256) NOT NULL, 
    `f_version` VARCHAR(40) NOT NULL,
    `f_svc_url` text NOT NULL,  
    `f_description` text, 
    `f_path` text NOT NULL,
    `f_method` VARCHAR(50) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `t_op_registry` (
    `f_id` SERIAL NOT NULL,
    `f_op_id` VARCHAR(40) NOT NULL,
    `f_name` VARCHAR(512) NOT NULL, 
    `f_metadata_version` VARCHAR(40) NOT NULL,
    `f_is_latest` TINYINT(1)  DEFAULT 0, 
    `f_metadata_type` VARCHAR(40) NOT NULL,
    `f_status` VARCHAR(10) DEFAULT 0,
    `f_operator_type` VARCHAR(10) DEFAULT 0,
    `f_execution_mode` VARCHAR(10) DEFAULT 0,
    `f_category` VARCHAR(50) DEFAULT 0,
    `f_source` VARCHAR(50) DEFAULT '',
